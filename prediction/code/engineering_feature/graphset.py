...
#             os.makedirs(self.processed_dir)
    ...
    # def process(self, graphs_dict):
    #     data_list = []
    #     for graph_name, graph_data in graphs_dict.items():
    #         features = graph_data['features']  # 假设特征数据存储在名为'features'的键下
    #         edge_index = graph_data['edge_index']  # 假设边索引数据存储在名为'edge_index'的键下
    #         GCNData = DATA.Data(x=torch.Tensor(features), edge_index=torch.LongTensor(edge_index))
    #         data_list.append(GCNData)
    #     self.data = data_list
